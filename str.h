...
namespace cmplx {
	...
	static unsigned_t strlen(const char* str) {
		unsigned_t len; for (len = 0; *(str + len) != '\0'; len++);
		return len;
	}
	...
	public:
		...
		~String() { if (!doNotDestroy) delete[] strPtr; };
		...
		String(const String& str) { init(str.strPtr); }
		String(integer_t num) { init(itoa(num)); }
		String(int num) { init(itoa(num)); }
		String(double num) {
		...
		String operator+(int num) { return operator+(itoa(num)); }
		String operator+(void* addr) { return operator+(hextoa(addr)); }
		String operator+(char ch) { char x[2] = { ch, 0 }; return operator+(x); }
